...
> 基于 Python 学习实践持续整理，涵盖入门第 3 章"数据类型与运算符"（01-18 全部小节）、IBM 课程各节（字符串操作、正则表达式、列表和元组、集合、词典）、教程第 4 章流程控制（已完结：分支、循环、九九乘法表案例、continue 与 break、闯关游戏案例）及教程第 5 章函数（函数、参数），分为"基本语法元素"、"数据类型"、"数据类型转换"、"运算符"、"输入语句"、"字符串操作"、"正则表达式"、"列表和元组"、"集合"、"词典"、"流程控制语句——分支"、"流程控制语句——while循环"、"流程控制语句——for循环"、"案例——九九乘法表"、"continue 与 break"、"案例——闯关游戏"、"函数"与"参数"十八大部分。
