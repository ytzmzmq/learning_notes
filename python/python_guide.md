...
> 基于 Python 学习实践持续整理，涵盖入门第 3 章"数据类型与运算符"（01-18 全部小节）、IBM 课程各节（字符串操作、正则表达式、列表和元组、集合、词典）及教程第 4 章流程控制（分支、循环），分为"基本语法元素"、"数据类型"、"数据类型转换"、"运算符"、"输入语句"、"字符串操作"、"正则表达式"、"列表和元组"、"集合"、"词典"、"流程控制语句——分支"、"流程控制语句——while循环"与"流程控制语句——for循环"十三大部分。
