...
> 基于 Python 学习实践持续整理，涵盖入门第 3 章"数据类型与运算符"（01-18 全部小节）、IBM 课程各节（字符串操作、正则表达式、列表和元组、集合、词典）、教程第 4 章流程控制（已完结：分支、循环、九九乘法表案例、continue 与 break、闯关游戏案例）及教程第 5 章函数（函数、参数、字面量 None 与返回值、全局作用域与局部作用域），分为"基本语法元素"、"数据类型"、"数据类型转换"、"运算符"、"输入语句"、"字符串操作"、"正则表达式"、"列表和元组"、"集合"、"词典"、"流程控制语句——分支"、"流程控制语句——while循环"、"流程控制语句——for循环"、"案例——九九乘法表"、"continue 与 break"、"案例——闯关游戏"、"函数"、"参数"、"字面量 None 与返回值"与"全局作用域与局部作用域"二十大部分。
...
函数的优点：提高代码复用性、结构清晰更易读、便于模块化开发。

按来源分为三类：内置函数（如 `print`、`len`）、模块提供的函数（如 `random.randint`）、自定义函数。

## 17.2 定义与调用

用 `def` 定义函数，函数名后加括号和冒号，函数体缩进书写；定义只是声明，**调用（函数名加括号）时才执行**：
...
**位置参数：** 按参数定义的顺序，依次把实参的值传递给形参。

```python
def yt_score(course, score, level):
    print(f"yt在课程{course}的成绩是{score}，评价是{level}")

yt_score("数学", 150, "A")
```

**关键字参数：** 用"形参名=值"的形式传参，不需要考虑顺序，但不能重复传同一个参数。位置参数与关键字参数混用时，**位置参数必须在前**。

```python
yt_score("政治", level="A", score=98)   # 关键字参数顺序随意
```
关键字参数的 `=` 左右官方建议不加空格。

## 17.4 限制传参方式

定义函数时可以用 `/` 和 `*` 限制调用方的传参方式：`/` 前面的参数只能用位置传参，`*` 后面的参数只能用关键字传参，两者不需要同时使用。

```python
def wzt_score(course, /, score, level, *, university):
    print(f"wzt在{course}课程的成绩是{score}，等级是{level}，在{university}获得")

wzt_score("数学", university="浙江大学", level="A", score=99)
```
上例中 `course` 只能按位置传，`university` 只能按关键字传，`score`、`level` 两种方式都可以。
...
定义函数时用"形参名=值"为参数设置默认值。**默认参数必须写在必选参数的后面**；调用时不传就取默认值。

```python
def yt_score(course, score=100, level="A"):
    print(f"yt在{course}课程中取得了{score}分，评级是{level}")

yt_score("数学", level="A+")   # score 取默认值 100
```

## 18.2 可变位置参数（*）

形参名前加 `*`，可以接收任意数量的位置参数，并打包成一个**元组**。习惯用 `args`（arguments）命名：

```python
def test1(*args):
    print(args)

test1("yt", "wzt", "nabo", "ox")   # ('yt', 'wzt', 'nabo', 'ox')
```

## 18.3 可变关键字参数（**）

形参名前加 `**`，可以接收任意数量的关键字参数，并打包成一个**字典**。习惯用 `kwargs`（keyword arguments）命名：

```python
def test2(**kwargs):
    print(kwargs)

test2(name="wzt", level="A+", score=99)   # {'name': 'wzt', 'level': 'A+', 'score': 99}
```

## 18.4 多种参数混合使用

同时使用可变位置参数和可变关键字参数时，**先写 `*args` 再写 `**kwargs`**：

```python
def test3(*args, **kwargs):
    print(args)      # ('yt', 'ox', 'wzt')
    print(kwargs)    # {'score': 99, 'level': 'A+', 'name': 'nabo'}

test3("yt", "ox", "wzt", score=99, level="A+", name="nabo")
```

与必选参数、默认参数一起使用时，顺序为：必选 → `*args` → 默认 → `**kwargs`：

```python
def test4(name, *args, score=100, **kwargs):
    print(name, score)
    print(args)
    print(kwargs)

test4("yt", "ox", "wzt", score=99, teacher="nabo", ytzmzmq="correct")
# yt 99 / ('ox', 'wzt') / {'teacher': 'nabo', 'ytzmzmq': 'correct'}
test4("yt", "ox", "wzt", teacher="nabo", ytzmzmj="correct")
# yt 100（score 取默认值）
```
注意：`*args` 后面的 `score=100` 实际上只能按关键字传参（效果类似 `*` 之后的限制）。
...
`NoneType` 类型只有一个值 `None`，表示空值、无意义。常用于占位（如先声明变量稍后再赋值）；如果已经确定数据类型，就不需要用 None。None 不暗示任何数据类型。

```python
msg = None
print(type(msg), msg)   # <class 'NoneType'> None
```

None 的特性：
- 转 bool 是 False，因此可以直接配合 `not` 做判断。
- **不能进行数学运算，也不能进行字符串拼接**，会报 TypeError。

```python
print(bool(msg))    # False
if not msg:
    print("yt")     # msg 为假，打印 yt
```

## 19.2 函数返回值

函数执行完毕后，会把执行结果交给调用者，这个结果就是函数的返回值。**不给函数设置返回值时，函数默认返回 None。**

`return` 做两件事：结束函数的运行，并把 return 后面的值作为返回值。只要函数的计算结果在后面的代码中还要用到，就必须使用 return。

```python
def add(a, b):
    answer = a + b
    print(answer, "打印完毕")
    return answer     # 结束函数，返回 answer

def sub(a, b):
    return a - b      # 直接返回表达式的值

result1 = add(3, 4)
print(f"add函数的返回值是{result1}")   # add函数的返回值是7
```
`return` 之后的语句不会执行，函数到此为止。
...
作用域指变量能起作用的范围：
- **全局作用域**：整个 .py 文件最外层的范围，其中定义的是全局变量。
- **局部作用域**：只能在当前函数内部使用，其中定义的是局部变量。

## 20.2 变量查找顺序与遮蔽

函数内部会**先在局部作用域查找变量，找不到再去外层（全局）作用域查找**：
...
def test1():
    c = 300
    d = 400
    e = 555        # 在函数内新建了局部变量 e，遮蔽（不影响）全局的 e
    global f       # 声明 f 是全局变量，下面修改的就是全局的 f
    f = 666
    print("函数中ab的打印", a, b)   # 100 200，局部找不到，去全局找
    print("函数中cd的打印", c, d)   # 300 400，局部变量正常打印
    print("函数中e的打印", e)       # 555，打印的是局部的 e
    print("函数中f的打印", f)       # 666，global 声明后用的是全局的 f


test1()
print("全局作用域ab的打印", a, b)   # 100 200
print("全局作用域e的打印", e)       # 500，局部 e 没有影响全局 e
print("全局作用域f的打印", f)       # 666，被函数内修改了
```

要点：
- 函数内直接给变量赋值（如 `e = 555`）会**创建同名局部变量**，遮蔽全局变量，全局值不受影响。
- 要在函数内修改全局变量，必须先用 `global` 声明。
- 函数执行结束后，函数内的局部变量（如 c、d）在外部无法访问。

## 20.3 作用域的生命周期

- **局部作用域和局部变量**在函数调用时创建，函数执行结束后销毁——所以 `test2()` 调用两次，`m` 每次都从 10 重新开始。
- **全局作用域和全局变量**在程序开始时创建，程序结束后销毁。
...
| 语法 | 含义 |
|------|------|
| `def 函数名(参数):` | 定义函数，调用时才执行 |
| 位置参数 | 按定义顺序依次传值 |
| `形参=值` | 关键字传参 / 默认值（默认参数在必选参数后） |
| `参数, /` | `/` 前的参数只能按位置传 |
| `*, 参数` | `*` 后的参数只能按关键字传 |
| `*args` | 可变位置参数，打包为元组 |
| `**kwargs` | 可变关键字参数，打包为字典 |
| 混合顺序 | 必选 → `*args` → 默认 → `**kwargs` |
...
|------|------|
| `return 值` | 结束函数并返回该值 |
| 无 return / 裸 return | 函数返回 None |
| `return a, b` | 返回元组 (a, b) |
| 全局变量 | 文件最外层定义，程序结束销毁 |
| 局部变量 | 函数内定义，调用时创建、结束销毁 |
| `global 变量` | 函数内声明使用全局变量 |
| 查找顺序 | 先局部后全局（LEGB） |
...
- 函数定义只是声明，调用（加括号）时才执行；忘写括号不会报错但函数体不会运行。
- 位置参数与关键字参数混用时，位置参数必须在前；关键字参数不能重复传同一个形参。
- 默认参数必须写在必选参数后面，否则定义时就报 SyntaxError。
- `*args` 收到的是元组，`**kwargs` 收到的是字典；`args`/`kwargs` 只是习惯命名，不是关键字。
- 多种参数混合定义的顺序：必选 → `*args` → 默认值参数 → `**kwargs`，写错顺序会报错。
- `/` 和 `*` 是传参方式限制符：`/` 前只能位置传参，`*` 后只能关键字传参。
- `None` 不能参与数学运算或字符串拼接，会报 TypeError；`bool(None)` 是 False。
- 函数不写 return 就默认返回 None，打印"没返回值的函数"结果常因此显示 None。
- `return` 会立即结束函数，写在 return 之后的语句永远不执行。
- 函数内直接给变量赋值会创建同名局部变量并遮蔽全局变量，全局值不变；要改全局必须先 `global` 声明。
- 局部变量在函数结束后销毁，函数外部访问不到（NameError）。
- 变量查找先局部后全局：函数内能读全局变量，但同名局部变量会优先。
